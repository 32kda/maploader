...
public class XmlStreamParsingException extends XMLStreamException {
    ...
    public String getMessage() {
        ...
        StringBuilder sb = new StringBuilder(msg).append(' ')
                .append(MessageFormat.format("(at line {0}, column {1})", getLocation().getLineNumber(), getLocation().getColumnNumber()));
        int offset = getLocation().getCharacterOffset();
        if (offset > -1) {
            sb.append(". ").append(MessageFormat.format("{0} bytes have been read", offset));
        }
        return sb.toString();
    }
}
