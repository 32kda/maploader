...
public final class OsmChangesetParser {
    ...
    private class Parser extends DefaultHandler {
        ...
        protected void parseChangesetAttributes(Attributes atts) throws XmlParsingException {
            ...
            if (value == null) {
                throwException(MessageFormat.format("Missing mandatory attribute ''{0}''.", "id"));
            }
            ...
            if (value == null) {
                throwException(MessageFormat.format("Missing mandatory attribute ''{0}''.", "open"));
            } else if ("true".equals(value)) {
                current.setOpen(true);
            } else if ("false".equals(value)) {
                current.setOpen(false);
            } else {
                throwException(MessageFormat.format("Illegal boolean value for attribute ''{0}''. Got ''{1}''.", "open", value));
            }
            ...
            if (minLonStr != null && minLatStr != null && maxLonStr != null && maxLatStr != null) {
                ...
                try {
                    ...
                    throwException(MessageFormat.format("Illegal value for attribute ''{0}''. Got ''{1}''.", "min_lon", minLonStr));
                }
                double minLat = 0;
                try {
                    minLat = Double.parseDouble(minLatStr);
                } catch (NumberFormatException e) {
                    throwException(MessageFormat.format("Illegal value for attribute ''{0}''. Got ''{1}''.", "min_lat", minLatStr));
                }
                ...
                try {
                    ...
                    throwException(MessageFormat.format("Illegal value for attribute ''{0}''. Got ''{1}''.", "max_lon", maxLonStr));
                }
                double maxLat = 0;
                try {
                    maxLat = Double.parseDouble(maxLatStr);
                } catch (NumberFormatException e) {
                    throwException(MessageFormat.format("Illegal value for attribute ''{0}''. Got ''{1}''.", "max_lat", maxLatStr));
                }
        ...
        private int parseNumericAttribute(String value, int minAllowed) throws XmlParsingException {
            ...
            try {
                ...
                throwException(MessageFormat.format("Illegal value for attribute ''{0}''. Got ''{1}''.", "id", value));
            }
            if (att < minAllowed) {
                throwException(MessageFormat.format("Illegal numeric value for attribute ''{0}''. Got ''{1}''.", "id", att));
            }
            return att;
        }
        ...
        public void startElement(String namespaceURI, String localName, String qName, Attributes atts) throws SAXException {
            ...
            case "osm":
                if (atts == null) {
                    throwException(MessageFormat.format("Missing mandatory attribute ''{0}'' of XML element {1}.", "version", "osm"));
                    return;
                }
                String v = atts.getValue("version");
                if (v == null) {
                    throwException(MessageFormat.format("Missing mandatory attribute ''{0}''.", "version"));
                }
                if (!("0.6".equals(v))) {
                    throwException(MessageFormat.format("Unsupported version: {0}", v));
                }
                break;
            case "changeset":
            ...
            case "text":
                ...
                break;
            default:
                throwException(MessageFormat.format("Undefined element ''{0}'' found in input stream. Aborting.", qName));
            }
        }
    ...
    public static List<Changeset> parse(InputStream source, ProgressMonitor progressMonitor) throws IllegalDataException {
        ...
        try {
            ...
            progressMonitor.indeterminateSubTask(MessageFormat.format("Parsing list of changesets..."));
