...
public class OsmServerUserPreferencesReader extends OsmServerReader {
        ...
        try {
            ...
            if (xmlNodeList != null) {
                for (int i = 0; i < xmlNodeList.getLength(); i++) {
                    ...
                    if (k == null)
                        throw new XmlParsingException(MessageFormat.format("Missing attribute ''{0}'' on XML tag ''{1}''.", "k", "preference"));
                    String v = getAttribute(xmlNode, "v");
                    if (v == null)
                        throw new XmlParsingException(MessageFormat.format("Missing attribute ''{0}'' on XML tag ''{1}''.", "v", "preference"));
                    result.put(k, v);
                }
            }
    ...
    public Map<String, String> fetchUserPreferences(ProgressMonitor monitor, String reason) throws OsmTransferException {
        return fetchData("user/preferences", MessageFormat.format("Reading user preferences ..."),
