    ...
    public List<String> parseOptions(List<String> arguments) {
                ...
                if (definition.requiresParameter()) {
                    if (split.length > 1) {
                        ...
                        if (toHandle.isEmpty() || "--".equals(toHandle.getFirst())) {
                            throw new OptionParseException(MessageFormat.format("{0}: option ''{1}'' requires an argument", program, optionName));
                        }
                        parameter = toHandle.removeFirst();
                    }
                } else if (split.length > 1) {
                    throw new OptionParseException(
                            MessageFormat.format("{0}: option ''{1}'' does not allow an argument", program, optionName));
                }
        ...
        availableOptions.values().stream().distinct().forEach(def -> {
            ...
            if (count < def.getRequiredCount().min) {
                ...
                throw new OptionParseException(MessageFormat.format("{0}: option ''{1}'' is required", program, optionName));
            } else if (count > def.getRequiredCount().max) {
                // max may be 1 or MAX_INT at the moment
                throw new OptionParseException(MessageFormat.format("{0}: option ''{1}'' may not appear multiple times", program, optionName));
            }
        });
        ...
        for (FoundOption option : options) {
            try {
                ...
                if (option.parameter == null) {
                    message.append(MessageFormat.format("{0}: Error while handling option ''{1}''", program, option.optionName));
                } else {
                    message.append(MessageFormat.format("{0}: Invalid value {2} for option ''{1}''", program, option.optionName,
                            option.parameter));
                }
    ...
    private AvailableOption findParameter(String optionName) {
        ...
        if (exactMatch != null) {
            ...
            if (alternatives.size() == 1) {
                ...
                throw new OptionParseException(MessageFormat.format("{0}: option ''{1}'' is ambiguous", program, optionName));
            }
        }
        throw new OptionParseException(MessageFormat.format("{0}: unrecognized option ''{1}''", program, optionName));
    }
