...
public class DefaultProxySelector extends ProxySelector {
    ...
    protected int parseProxyPortValue(String property, String value) {
        ...
        try {
            ...
            Logging.error(MessageFormat.format("Unexpected format for port number in preference ''{0}''. Got ''{1}''.", property, value));
            Logging.error(MessageFormat.format("The proxy will not be used."));
            return 0;
        }
        if (port <= 0 || port > 65_535) {
            Logging.error(MessageFormat.format("Illegal port number in preference ''{0}''. Got {1}.", property, port));
            Logging.error(MessageFormat.format("The proxy will not be used."));
            return 0;
        }
        return port;
    ...
    public final void initFromPreferences() {
        ...
        if (value.isEmpty()) {
            ...
            if (proxyPolicy == null) {
                Logging.warn(MessageFormat.format("Unexpected value for preference ''{0}'' found. Got ''{1}''. Will use no proxy.",
                        PROXY_POLICY, value));
                proxyPolicy = ProxyPolicy.NO_PROXY;
            }
        }
        ...
        if (proxyPolicy == ProxyPolicy.USE_HTTP_PROXY) {
            if (host != null && !host.trim().isEmpty() && port > 0) {
                ...
                Logging.warn(MessageFormat.format("Unexpected parameters for HTTP proxy. Got host ''{0}'' and port ''{1}''.", host, port));
                Logging.warn(MessageFormat.format("The proxy will not be used."));
            }
        }
        ...
        if (proxyPolicy == ProxyPolicy.USE_SOCKS_PROXY) {
            if (host != null && !host.trim().isEmpty() && port > 0) {
                ...
                Logging.warn(MessageFormat.format("Unexpected parameters for SOCKS proxy. Got host ''{0}'' and port ''{1}''.", host, port));
                Logging.warn(MessageFormat.format("The proxy will not be used."));
            }
        }
    ...
    public void connectFailed(URI uri, SocketAddress sa, IOException ioe) {
        ...
        Logging.error(MessageFormat.format("Connection to proxy ''{0}'' for URI ''{1}'' failed. Exception was: {2}",
    ...
    public List<Proxy> select(URI uri) {
        ...
        case USE_SYSTEM_SETTINGS:
            if (!jvmWillUseSystemProxies) {
                Logging.warn(MessageFormat.format("The JVM is not configured to lookup proxies from the system settings. "+
