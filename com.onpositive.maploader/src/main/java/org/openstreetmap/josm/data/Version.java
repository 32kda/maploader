...
public class Version {
    ...
    protected void initFromRevisionInfo(InputStream revisionInfo) {
        if (revisionInfo == null) {
            this.releaseDescription = "UNKNOWN";
        ...
        try {
            ...
            Logging.log(Logging.LEVEL_WARN, MessageFormat.format("Error reading revision info from revision file: {0}", e.getMessage()), e);
        }
        ...
        if (!value.isEmpty()) {
            try {
                ...
                version = 0;
                Logging.warn("Unexpected JOSM version number in revision file, value is ''{0}''", value);
            }
        } else {
    ...
    public void init() {
        try (InputStream stream = Utils.getResourceAsStream(getClass(), "/REVISION")) {
            if (stream == null) {
                Logging.warn("The revision file ''/REVISION'' is missing.");
                version = 0;
                releaseDescription = "";
                return;
            }
    ...
    public String getVersionString() {
        return version == 0 ? "UNKNOWN" : Integer.toString(version);
    }
