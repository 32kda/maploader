...
public class ProjectionCLI implements CLIModule {
    ...
    public void processArguments(String[] argArray) {
        ...
        try {
            ...
            System.err.println(MessageFormat.format("Error: {0}", ex.getMessage()));
            System.exit(1);
        }
        System.exit(0);
    }
    ...
    private static String getHelp() {
        return MessageFormat.format("JOSM projection command line interface")+"\n\n"+
                MessageFormat.format("Usage")+":\n"+
                "\tjava -jar josm.jar project <options> <crs> +to <crs> [file]\n\n"+
                MessageFormat.format("Description")+":\n"+
                MessageFormat.format("Converts coordinates from one coordinate reference system to another.")+"\n\n"+
                MessageFormat.format("Options")+":\n"+
                "\t--help|-h         "+MessageFormat.format("Show this help")+"\n"+
                "\t-I                "+MessageFormat.format("Switch input and output crs")+"\n"+
                "\t-r                "+MessageFormat.format("Switch order of input coordinates (east/north, lon/lat)")+"\n"+
                "\t-s                "+MessageFormat.format("Switch order of output coordinates (east/north, lon/lat)")+"\n\n"+
                MessageFormat.format("<crs>")+":\n"+
                MessageFormat.format("The format for input and output coordinate reference system"
                        + " is similar to that of the PROJ.4 software.")+"\n\n"+
                MessageFormat.format("[file]")+":\n"+
                MessageFormat.format("Reads input data from one or more files listed as positional arguments. "
                + "When no files are given, or the filename is \"-\", data is read from "
                + "standard input.")+"\n\n"+
                MessageFormat.format("Examples")+":\n"+
    ...
    private EastNorth parseEastNorth(String s, ToDoubleFunction<String> parser) {
        String[] en = s.split("[;, ]+");
        if (en.length != 2)
            throw new IllegalArgumentException(MessageFormat.format("Expected two coordinates, separated by white space, found {0} in ''{1}''", en.length, s));
    ...
    private static double parseDouble(String s) {
        try {
            ...
            throw new IllegalArgumentException(MessageFormat.format("Unable to parse number ''{0}''", s), nfe);
        }
    }
