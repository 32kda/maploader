...
public class UrlValidator extends AbstractValidator {
    ...
    public boolean isValid(String value) {
        ...
        if (!urlMatcher.matches()) {
            setErrorMessage(MessageFormat.format("URL is invalid"));
            return false;
        }

        String scheme = urlMatcher.group(PARSE_URL_SCHEME);
        if (!isValidScheme(scheme)) {
            setErrorMessage(MessageFormat.format("URL contains an invalid protocol: {0}", scheme));
            return false;
        }
        ...
        if ("file".equals(scheme)) { // Special case - file: allows an empty authority
            if (!"".equals(authority) && authority.contains(":")) { // but cannot allow trailing :
                setErrorMessage(MessageFormat.format("URL contains an invalid authority: {0}", authority));
                return false;
            }
        ...
        if (!isValidPath(path)) {
            setErrorMessage(MessageFormat.format("URL contains an invalid path: {0}", path));
            return false;
        }

        String query = urlMatcher.group(PARSE_URL_QUERY);
        if (!isValidQuery(query)) {
            setErrorMessage(MessageFormat.format("URL contains an invalid query: {0}", query));
            return false;
        }

        String fragment = urlMatcher.group(PARSE_URL_FRAGMENT);
        if (!isValidFragment(fragment)) {
            setErrorMessage(MessageFormat.format("URL contains an invalid fragment: {0}", fragment));
            return false;
        }
    ...
    public String getValidatorName() {
        return MessageFormat.format("URL validator");
    }
