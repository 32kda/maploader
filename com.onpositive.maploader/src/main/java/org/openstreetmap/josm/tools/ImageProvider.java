...
public class ImageProvider {
        ...
        if (ir == null) {
            if (!optional) {
                ...
                throw new JosmRuntimeException(
                		MessageFormat.format("Fatal: failed to locate image ''{0}''. This is a serious configuration problem. JOSM will stop working.",
                                name + ext));
            } else {
                if (!suppressWarnings) {
                    Logging.error(MessageFormat.format("Failed to locate image ''{0}''", name));
                }
                return null;
            }
        }
    ...
    private static ImageResource getIfAvailableZip(String fullName, File archive, String inArchiveDir, ImageType type) {
        try (ZipFile zipFile = new ZipFile(archive, StandardCharsets.UTF_8)) {
            ...
            Logging.log(Logging.LEVEL_WARN, MessageFormat.format("Failed to handle zip file ''{0}''. Exception was: {1}", archive.getName(), e.toString()), e);
        }
        return null;
    }
        ...
        if (dirs != null) {
            for (String name : dirs) {
                try {
                    ...
                    Logging.log(Logging.LEVEL_WARN, MessageFormat.format(
...
//        }
    ...
    public static Cursor getCursor(String name, String overlay) {
        ...
        if (overlay != null) {
            img = new ImageProvider("cursor", name).setMaxSize(ImageSizes.CURSOR).setMaxSize(ImageSizes.CURSOROVERLAY).get();
        }
