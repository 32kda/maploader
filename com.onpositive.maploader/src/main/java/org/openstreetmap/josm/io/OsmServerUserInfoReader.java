...
public class OsmServerUserInfoReader extends OsmServerReader {
    ...
    public static UserInfo buildFromXML(Document document) throws XmlParsingException {
        try {
            ...
            if (xmlNode == null)
                throw new XmlParsingException(MessageFormat.format("XML tag <user> is missing."));

            // -- id
            String v = getAttribute(xmlNode, "id");
            if (v == null)
                throw new XmlParsingException(MessageFormat.format("Missing attribute ''{0}'' on XML tag ''{1}''.", "id", "user"));
            try {
                userInfo.setId(Integer.parseInt(v));
            } catch (NumberFormatException e) {
                throw new XmlParsingException(MessageFormat.format("Illegal value for attribute ''{0}'' on XML tag ''{1}''. Got {2}.", "id", "user", v), e);
            }
            // -- display name
            ...
            if (xmlNode != null) {
                ...
                if (v == null)
                    throw new XmlParsingException(MessageFormat.format("Missing attribute ''{0}'' on XML tag ''{1}''.", "lat", "home"));
                double lat;
                try {
                    lat = Double.parseDouble(v);
                } catch (NumberFormatException e) {
                    throw new XmlParsingException(MessageFormat.format("Illegal value for attribute ''{0}'' on XML tag ''{1}''. Got {2}.",
                            "lat", "home", v), e);
                }

                v = getAttribute(xmlNode, "lon");
                if (v == null)
                    throw new XmlParsingException(MessageFormat.format("Missing attribute ''{0}'' on XML tag ''{1}''.", "lon", "home"));
                double lon;
                try {
                ...
                if (v == null)
                    throw new XmlParsingException(MessageFormat.format("Missing attribute ''{0}'' on XML tag ''{1}''.", "zoom", "home"));
                int zoom;
                try {
            ...
            if (xmlNode != null) {
                ...
                if (v == null)
                    throw new XmlParsingException(MessageFormat.format("Missing attribute ''{0}'' on XML tag ''{1}''.", "unread", "received"));
                try {
                    userInfo.setUnreadMessages(Integer.parseInt(v));
                } catch (NumberFormatException e) {
                    throw new XmlParsingException(
                            MessageFormat.format("Illegal value for attribute ''{0}'' on XML tag ''{1}''. Got {2}.", "unread", "received", v), e);
                }
            }
    ...
    public UserInfo fetchUserInfo(ProgressMonitor monitor, String reason) throws OsmTransferException {
        return fetchData("user/details", MessageFormat.format("Reading user info ..."),
